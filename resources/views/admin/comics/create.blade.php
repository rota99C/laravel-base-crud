...
<form action="{{route('admin.comics.store')}}" method="post">
    ...
    <div class="mb-4 ms-4 me-4">
        ...
        <input value="{{old('sale_date')}}" class="form-control @error('title') is-invalid @enderror mb-4" type="date" name="sale_date" id="sale_date" value="2022-01-20" aria-describedby="helpId">
        @error('title')
        <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        <label for="type" class="form-label mb-2 text-secondary"><strong>Comic type</strong> </label>
        <input value="{{old('type')}}" class="form-control @error('title') is-invalid @enderror mb-4" type="text" name="type" id="type" placeholder="Insert here the type of the comic" aria-describedby="helpId">
        @error('title')
        <div class="alert alert-danger">{{ $message }}</div>
        @enderror
